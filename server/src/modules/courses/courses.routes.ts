...
export async function courseRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.put("/:id", { preHandler: [requireRole("TEACHER", "ADMIN")] }, async (request, reply) => {
    const { id } = request.params as { id: string };
  ...
  app.delete("/:id", { preHandler: [requireRole("TEACHER", "ADMIN")] }, async (request, reply) => {
    const { id } = request.params as { id: string };
    await service.deleteCourse(id);
    reply.send({ success: true, message: "Course deleted" });
  });

  app.post("/:id/publish", { preHandler: [requireRole("TEACHER", "ADMIN")] }, async (request, reply) => {
    const { id } = request.params as { id: string };
